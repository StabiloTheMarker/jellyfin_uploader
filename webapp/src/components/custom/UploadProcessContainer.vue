...
async function onProcessDeletedClicked(processId: number) {
  const response = await fetch("/api/upload_process?processId=" + processId, {
    method: "DELETE",
  })
  if (!response.ok) {
...
<template>
  <div class="border border-gray-700 rounded-md p-2">
    <div class="flex justify-between items-center mb-3">
      ...
      <Button @click="() => onProcessDeletedClicked(uploadProcess.ID)" variant="destructive" size="icon" class="cursor-pointer">
        <Trash :size="15"/>
      </Button>
    </div>
